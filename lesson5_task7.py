...
with open('firms.txt') as f:
    firms = f.readlines()
    for firm in firms:
        tmp = firm.strip().split()
        fin_res.update({tmp[0]: int(tmp[2]) - int(tmp[3])})

profits = [i for i in fin_res.values() if i > 0]
my_list = [fin_res, {'average_profit': round(sum(profits) / len(profits))}]
